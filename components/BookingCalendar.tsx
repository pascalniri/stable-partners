...
import { format, isSameDay, parseISO, isBefore, isAfter, startOfDay } from "date-fns";
...
export default function BookingCalendar({ onSelect, selectedSlotId }: BookingCalendarProps) {
  ...
  const availableDays = availableSlots.map(s => startOfDay(parseISO(s.startTime)));
  const slotsForSelectedDay = availableSlots.filter((slot) =>
    selectedDay ? isSameDay(parseISO(slot.startTime), selectedDay) : false
  );

  return (
    <div className="flex flex-col lg:flex-row gap-8 bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm">
      {/* Date Selection */}
      <div className="p-6 bg-slate-50/50 border-r border-slate-100">
        ...
        <div className="mt-6">
          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Your Timezone</label>
          <select 
            value={timezone} 
            onChange={(e) => setTimezone(e.target.value)}
            className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium"
          >
            {timezones.map(tz => (
      ...
      <div className="flex-1 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          {selectedDay && (
            <span className="text-xs font-medium text-slate-500">{format(selectedDay, "EEEE, MMMM do")}</span>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {slotsForSelectedDay.map((slot) => (
              ...
              >
                ...
                <ChevronRight size={14} className={selectedSlotId === slot.id ? "text-white" : "text-slate-300 group-hover:text-[#1800AC]"} />
              </button>
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center h-64 text-center p-8 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            <Clock size={32} className="text-slate-300 mb-4" />
            <p className="text-sm font-bold text-slate-500 mb-1">No slots available for this day</p>
            <p className="text-xs text-slate-400">Please select another date on the calendar.</p>
          </div>
        )}
      </div>
    </div>
